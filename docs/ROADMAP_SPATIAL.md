...
| **Execution isolation** | `ExecBackend`: gVisor/Docker → configured SSH edge workers; host execution requires explicit opt-in |
...
Expose the companion with HTTPS (for example, `tailscale serve --bg http://127.0.0.1:8766`), open `https://JARVIS-PC.YOUR-TAILNET.ts.net/spatial?token=THE_COMPANION_TOKEN`, then press **Enter Spatial HUD**. The bootstrap redirects to a token-free URL with a 12-hour HttpOnly session. Quest requires HTTPS for immersive WebXR; the flat HUD remains usable over ordinary local HTTP.
...
Set `exec_backend` to `edge` to require an edge target, or `auto` to try Docker first. `exec_host_fallback` defaults to `false`; explicit `host` mode is available for trusted jobs but is not a security boundary.
...
### MQTT state bus
Run `homeassistant/start_homeassistant.bat`, then set `mqtt_enabled: true`. Mosquitto is non-persistent and bound to loopback on port `1883`; retained telemetry excludes conversation text. The companion API continues serving state if the broker is down.
...
Chroma and Hub markdown memory carry sensitivity labels. Hub events default to `personal`; personal events are blocked from Slack/n8n. Explicitly labeled `work` events are allowed by default, while `JARVIS_SHARE_MAX_SENSITIVITY=public` restricts external fan-out to public events only.
