#!/usr/bin/env python3
"""Night Log — daily journal CLI that writes markdown files. Built by Jarvis vibe agent."""
...
def add_entry(text: str) -> None:
    ...
    block = f"## {stamp}\n\n{text.strip()}\n\n"
    prev = LOG.read_text(encoding="utf-8") if LOG.exists() else "# Night Log\n\n"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Night Log: daily journal CLI that writes markdown files")
