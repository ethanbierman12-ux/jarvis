...
powershell -NoProfile -Command ^
  "$body = @{ target = 'ipad'; context = 'PC shutting down'; project = (Get-Location).Path } | ConvertTo-Json;" ^
  "try { Invoke-RestMethod -Method Post -Uri ('http://127.0.0.1:%PORT%/api/handoff?token=' + $env:COMPANION_TOKEN) -ContentType 'application/json' -Body $body -TimeoutSec 4 } catch { $_.Exception.Message }"
